...
| 파일 | 역할 |
|---|---|
| `index.html` | 메인 화면(난이도 선택) + 게임 화면 두 개를 하나에 담음. `#scrollRoot`로 콘텐츠 감쌈 |
| `style.css` | Win95 클래식 스타일 (회색 베벨, 빨간 LED, 9색 숫자) |
| `script.js` | 게임 로직 + 입력 처리 + 사운드 + 화면 전환 + PWA 등록 |
| `manifest.json` | PWA 메타데이터 (앱 이름, 아이콘, 색상 등) |
| `service-worker.js` | 오프라인 캐시 + 자동 갱신. **새 배포 때 `CACHE` 버전 숫자 올림** |
| `icon.svg` | 폭탄 디자인 벡터 아이콘 (소스) |
| `icon-192.png`, `icon-512.png` | PWA용 PNG 아이콘 (rsvg-convert로 SVG에서 변환) |
...
- 다크/라이트 테마 토글
- 게임 진행 중 메뉴로 돌아가도 상태 유지 (지금은 새 판)
